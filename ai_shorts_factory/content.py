...
def generate_topic(avoid: list[str] | None = None) -> str:
    ...
    prompt = (
        ...
        "- 4-9 words, Title Case, no quotes, no emojis, no clickbait lies.\n"
...
def generate_script(topic: str) -> list[Scene]:
    ...
    prompt = (
        ...
        f"{_theme_text()}.\n\n"
        f'Write a script for a ~35-45 second vertical Short titled: "{topic}".\n\n'
        "Engineer it for the Shorts algorithm (viewed-vs-swiped, watch-through, "
        "replays, shares). Hard rules:\n"
        f"- Exactly {n} scenes.\n"
        "- SCENE 1 = THE HOOK. The first 3-4 spoken words must deliver an "
        "instant shock or curiosity gap so no one swipes away. State the most "
        "fascinating promise immediately. Max 12 words. Never start with a "
        "greeting, 'imagine', or slow setup.\n"
        "- SCENE 2 = RE-HOOK: deepen the mystery or raise the stakes so the "
        "viewer commits to staying.\n"
        "- MIDDLE scenes: escalate tension with ONE concrete, vivid, genuinely "
        "surprising fact or number per scene (real science, no vague filler).\n"
        "- FINAL scene: land an ominous twist or a haunting open question, "
        "phrased so the last line flows naturally back into the hook (this loops "
        "the video and boosts replays). End on something people want to argue "
        "about or share.\n"
        "- Every narration line is 1-2 short, punchy, spoken sentences. No "
        "filler words, no 'in this video', no calls to like/subscribe.\n"
        f"- Language: {settings.content_language}.\n"
        "- For each scene write a vivid, cinematic AI IMAGE PROMPT (English, "
        "photorealistic, ultra-detailed, dramatic volumetric cinematic lighting, "
        "vertical 9:16 composition, no text, consistent dark dramatic mood that "
        "matches the narration of that exact scene).\n"
        "- 'on_screen_text' is a punchy 2-5 word caption for the scene.\n\n"
